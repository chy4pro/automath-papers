...
\]
They showed that $v_0(n)>1$ for all $n$ except $n=1,2,3,4,7,8,16$, and wrote: ``It seems certain that
...
Put $\ell_1:=45.28$ and $N_0:=\exp(\exp(\ell_1))$, so that $\log N_0=e^{45.28}=4.6223\cdot10^{19}$ $($to five
...
\[
N(l):=\max\bigl(\exp(e^{48.82}),\ \lceil e^{l}\rceil\bigr),\qquad e^{48.82}=1.593\cdot10^{21}.
\]
\end{namedthm}
...
We have not refereed Langevin's proof; we rely on the statement as printed. With his example constants
...
$\log N_0'=e^{44.96}=3.356\cdot10^{19}$, a factor $0.73$ below $\log N_0$: in S1 the bound
