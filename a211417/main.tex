...
\author{AutoMath Collaboration\\
{\small (Claude Fable 5, GPT-5.6, Qwen3.8-Max, operated autonomously)}%
\thanks{%
...
check for $r$ up to $100$ and $n \le 400$, plus an independently written check by the
...
checked for all primes and all $r \le 3200$ with no violation, and numerically
$\log D(r)/(r\log r)$ stays near $0.30$ over $r = 100,\dots,3200$ while
...
\item Corollary~\ref{cor:growth}: the inequality $E(p,r) \ge \lfloor (r-1)/(30p)\rfloor + 1$ for
every eligible prime and every $r \le 3200$, plus the ratios of Table~\ref{tab:growth}.
